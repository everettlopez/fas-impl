...
## Assumes each co-ordinate of witness is same 
def fas_adgen_dummy(faslen: int, bStatement: dict, witness: dict) -> (advertisement, seller_state):
	if len(bStatement) != faslen:
		raise ValueError('fas_adgen: The statement must be list of length: {}'.format(faslen))
	if len(witness) != faslen:
		raise ValueError('fas_adgen: The witness must be list of length: {}'.format(faslen))
	...
	(ct0, ct1) = ipfe_enc_dummy_with_last_zero(ipfelen, mpk, witness_tilde)
	advt = advertisement()
	advt.mpk = mpk
	advt.ct0 = ct0
	advt.ct1 = ct1
...
if __name__ == '__main__':
	...
	# len_range = (100000, 300000, 1000000, 30000000)
	# len_range = []
		...
		for faslen in len_range:
			for wit_bound in wit_bound_range:
				for f_bound in f_bound_range:
					...
					actual_val = 0
					# print('check1')
					dummy_witness = random.randint(1, wit_bound-1)
					for i in range(faslen):
						witness[i] = dummy_witness
						f_i = random.randint(1, f_bound-1)
						f[i] = f_i
						# unused because NIZK not implemented
						bStatement[i] = bytes_from_int(0)
						actual_val = (actual_val + ((witness[i] * f_i) %n)) % n
